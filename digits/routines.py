...
def mixmdan_train_routine(train_loader, test_pub_loader, test_priv_loader, loss_plt, cfg):
    device = 'cuda:0' if (cfg['use_cuda'] and torch.cuda.is_available()) else 'cpu'
    model = MDANet().to(device)
    alpha = nn.Parameter(torch.Tensor(len(train_loader.sources)))
    nn.init.kaiming_uniform_(alpha, a=np.sqrt(5))
    ...
    for epoch in range(cfg['epochs']):
        ...
        for i, (Xt, Xs, ys) in enumerate(train_loader):
            ...
            mix_domain_loss = torch.sum(beta * domain_losses)
            beta_no_grad = beta.copy()
            beta_no_grad.requires_grad_(False)
            mix_class_loss = torch.sum(beta_no_grad * class_losses)
            loss = mix_class_loss + cfg['mu']*mix_domain_loss + torch.sum(beta**2)/cfg['gamma']
